...
public class SachGUI extends JPanel implements ActionListener, MouseListener {
    ...
    private boolean check() {
        ...
        if (!CongCu.checkMa(txMasach.getText())) {
            JOptionPane.showMessageDialog(null, "Mã sách không chứa kí tự đặc biệt");
            return false;
        }
        else if(!CongCu.isLength15(txMasach.getText())){
            JOptionPane.showMessageDialog(null,"Mã sách không quá 15 kí tự");
            return false;
        }else if (!CongCu.checkNames(txTensach.getText())) {
            JOptionPane.showMessageDialog(null, "Tên sách chỉ gồm chữ và số");
            txTensach.requestFocus();
            return false;
        } else if (!CongCu.isLength50(txTensach.getText())) {
//            System.out.println("zo 50");
            JOptionPane.showMessageDialog(null, "Tên sách không được quá 50 ký tự");
            txTensach.requestFocus();
            return false;
